
const Home = () => {
  return (
    <>
      <div className="w-100 sm:px-28 p-0">
        <section className="w-100">
          <div className="w-100 px-4">
            <h4 className="text-center text-xl roboto-medium py-2 text-[#005BB3]">Acerca de la cátedra</h4>
            <p className="text-justify indent-6">La Metodología de la investigación aborda los pasos y enfoques necesarios para desarrollar investigaciones científicas de manera estructurada y precisa. Su objetivo es formar a los estudiantes en la recolección, análisis e interpretación de datos, así como en el uso de técnicas y herramientas adecuadas para abordar problemáticas de la realidad, permitiendo generar conocimiento válido y confiable.
            </p>
            <p className="text-justify indent-6 pt-2">
              Estas herramientas nos permitirá forjar nuestro espíritu crítico y posicionarnos profesionalmente mediante los análisis que pondrán en juego el pensamiento complejo, razonado y fundamentado, utilizando las técnicas y los métodos que nos servirán para explorar, describir, analizar y/o explicar científicamente los problemas de la realidad, y poder comunicarlos como parte de un mismo proceso cuyo producto será distinto de aquel que no posea dichas características.
            </p>
          </div>
        </section>
      </div>
    </>
  )
}

export default Home